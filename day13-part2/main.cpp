...
public:
    static ClawMachine parse(const std::string& data, int& pos)
    {
        ...
        pos += 10; // "\nPrize: X="
        Vector2i prize {};
        prize.x = parse_int(data, pos) + 10000000000000LL;
        pos += 4; // ", Y="
        prize.y = parse_int(data, pos) + 10000000000000LL;
...
private:
    ...
    [[nodiscard]] std::optional<Presses> presses_to_win() const
    {
        ...
        const int64_t b_num = -m_button_a.x * presses.button_a + m_prize.x;
        const int64_t b_denom = m_button_b.x;
        if (b_num % b_denom != 0) {
            return std::nullopt;
        }
...
int main()
{
    const std::string data = read_data("./day13-part2/input.txt");
