...
public:
    ...
    {
        constexpr std::array dirs { WordDir::north, WordDir::northeast, WordDir::east, WordDir::southeast,
                                    WordDir::south, WordDir::southwest, WordDir::west, WordDir::northwest };
        int count = 0;
        for (int y = 0; y < m_size.y; ++y) {
            for (int x = 0; x < m_size.x; ++x) {
                for (const WordDir dir : dirs) {
...
private:
    enum class WordDir { north, northeast, east, southeast, south, southwest, west, northwest };
    ...
    static Vector2i word_dir_offset(const WordDir dir)
    {
        switch (dir) {
        case WordDir::north:
            return { 0, -1 };
        case WordDir::northeast:
            return { 1, -1 };
        case WordDir::east:
            return { 1, 0 };
        case WordDir::southeast:
            return { 1, 1 };
        case WordDir::south:
            return { 0, 1 };
        case WordDir::southwest:
            return { -1, 1 };
        case WordDir::west:
            return { -1, 0 };
        case WordDir::northwest:
            return { -1, -1 };
        }
        std::unreachable();
    }

    [[nodiscard]] bool word_at(const std::string& word, const Vector2i& pos, const WordDir dir) const
    {
